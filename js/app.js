...
function rProg() {
  ...
  for (const k in PR) {
    ...
    sec.ex.forEach((ex, i) => {
      ...
      h += '<div class="ed"><span><span class="lb">Séries</span> ' + ex.sets + '×' + ex.reps + '</span><span><span class="lb">Leste</span> ' + wi + '</span><span><span class="lb">Repos</span> ' + (ex.rest >= 60 ? Math.floor(ex.rest / 60) + 'min' : ex.rest + 's') + '</span></div>';
      h += progHtml;
